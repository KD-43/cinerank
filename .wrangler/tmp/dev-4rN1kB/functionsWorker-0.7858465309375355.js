...
// .wrangler/tmp/pages-6X2Sdh/functionsWorker-0.7858465309375355.mjs
  ...
  "65b4acc3-e58e-41c7-85d1-de7ac3328054": [
    {
      ...
      timeDate: { dateCreated: "7-23-25", lastUpdated: "2:44 PM" },
      rows: [
...
__name(handleGetUserLists, "handleGetUserLists");
__name2(handleGetUserLists, "handleGetUserLists");
async function handleGetSpecificList(userId, listId) {
  const userLists = MOCK_DB[userId] || [];
  if (!userLists) {
    console.log(`[MOCK API]---Could NOT find lists for for user: ${userId[0, 3]}`);
  }
  const specificList = userLists.findIndex((l) => l.id === listId);
  if (specificList === -1 || !specificList) {
...
__name(handleGetSpecificList, "handleGetSpecificList");
__name2(handleGetSpecificList, "handleGetSpecificList");
async function handleSaveUserLists(userId, request) {
  console.log(`[MOCK API]---Saving lists for user: ${userId[0, 3]}`);
  const newListData = await request.json();
  const newListId = `tierList-id-${Math.random().toString(36).substring(2, 9)}`;
  if (!MOCK_DB[userId]) {
    MOCK_DB[userId] = [];
  }
  MOCK_DB[userId].push({ id: newListId, ...newListData });
  console.log(`[MOCK API]---Current data for user: ${userId[0, 3]}`, MOCK_DB[userId]);
  return new Response(JSON.stringify({ id: newListId, message: "List saved successfully (mocked)!" }), {
    status: 201,
    headers: { "Content-Type": "application/json" }
  });
}
__name(handleSaveUserLists, "handleSaveUserLists");
__name2(handleSaveUserLists, "handleSaveUserLists");
async function onRequest(context) {
  ...
  switch (request.method) {
    ...
    case "POST":
      return handleSaveUserLists(userId);
    default:
...
// .wrangler/tmp/bundle-snX34Y/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-snX34Y/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.7858465309375355.js.map
